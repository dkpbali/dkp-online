...
SHEET_MONITORING = "CB_MONITORING"
SHEET_BASELINE = "CB_BASELINE"
...
STATIONS = [
    ...
    {"name": "Selat Bali", "lat": -8.223, "lon": 114.437, "code": "SLB"}
]

def main():
    print("=== Start Coral Bleaching SST Anomaly Check ===")
    ...
    # 2. Read Baselines from CB_BASELINE sheet
    try:
        ...
        for idx in range(4, len(all_rows)):
            ...
            sst_mean = float(row[2])
            sst_std = float(row[3])
            baselines[(lokasi, bulan)] = (sst_mean, sst_std)
            
        print(f"Loaded {len(baselines)} SST baseline monthly records.")
    except Exception as e:
        print(f"Error loading SST baselines from sheet: {e}")
        traceback.print_exc()
        sys.exit(1)

    # 3. Read active incidents from CB_MONITORING to prevent duplicates
    try:
        ...
        print(f"Active incidents currently monitored in CB: {list(active_locations)}")
    except Exception as e:
        print(f"Error reading active incidents: {e}")
        traceback.print_exc()
        sys.exit(1)

    # 4. Fetch daily Sea Surface Temperature (SST) from Copernicus Marine
    # Get last 3 days to account for processing delays
    today = datetime.date.today()
    start_date = today - datetime.timedelta(days=3)
    
    print(f"Querying Copernicus for SST dates: {start_date} to {today}")
    ...
    if username and password:
        try:
            ...
            print("Successfully authenticated Copernicus credentials.")
        except Exception as e:
            print(f"Warning: Explicit login failed: {e}")

    nc_filename = "copernicus_sst_temp.nc"
    max_retries = 3
    for attempt in range(1, max_retries + 1):
        try:
            print(f"Attempting to download SST data from Copernicus (Attempt {attempt}/{max_retries})...")
            # Download subset NetCDF file for Sea Surface Temperature
            # Using Copernicus Global SST Daily Product
            ...
            print(f"Successfully downloaded satellite SST data to {nc_filename}")
            break
        except Exception as e:
            print(f"Error fetching SST data from Copernicus on attempt {attempt}: {e}")
    ...
    try:
        with xr.open_dataset(nc_filename) as ds:
            sst_var = 'analysed_sst' if 'analysed_sst' in ds.variables else 'sst'
            ...
            for station in STATIONS:
                ...
                if name in active_locations:
                    print(f"Station '{name}' has an active unresolved CB incident. Skipping check.")
                    continue
                ...
                # Get time series of SST at this point (in Kelvin)
                sst_ts = ds[sst_var].values[:, lat_idx, lon_idx]
                ...
                if measured_sst_k is None:
                    print(f"Station '{name}': No satellite SST data available (likely cloud-covered for all 3 days).")
                    continue
                    
                # Convert Kelvin to Celsius
                measured_sst_c = measured_sst_k - 273.15
                ...
                # Calculate SST anomaly (absolute deviation from climatological mean)
                sst_anomaly = measured_sst_c - base_mean
                print(f"Station '{name}': Measured SST = {measured_sst_c:.2f}°C | Baseline Mean = {base_mean:.2f}°C | SST Anomaly = +{sst_anomaly:.2f}°C")
                
                # Check threshold (anomaly > 1.0°C above climatological mean)
                if sst_anomaly > 1.0:
                    print(f"🚨 SST ANOMALY DETECTED AT {name.upper()}! Anomaly: +{sst_anomaly:.2f}°C > 1.0°C")
                    new_alerts.append({
                        "lokasi": name,
                        "lat": st_lat,
                        "lng": st_lon,
                        ...
                        "sst_value": round(measured_sst_c, 2),
                        "sst_baseline": round(base_mean, 2),
                        "anomaly_score": round(sst_anomaly, 2)
                    })
    except Exception as e:
    ...
    if new_alerts:
        ...
        try:
            ...
            for alert in new_alerts:
                event_id = f"CB-{date_prefix}-{alert['code']}"
                
                # Append row matching CB_MONITORING columns
                new_row = [
                    event_id,
                    alert["lokasi"],
                    alert["lat"],
                    ...
                    "",  # admin_verifikasi
                    "",  # tgl_status_final
                    "FALSE"  # alert_sent = FALSE
                ]
                
                ws_mon.append_row(new_row, value_input_option="USER_ENTERED")
                print(f"Created incident {event_id} for {alert['lokasi']} in CB_MONITORING sheet.")
                
        except Exception as e:
            print(f"Error writing to Google Sheets: {e}")
            traceback.print_exc()
    else:
        print("No new SST anomalies detected today.")
        
    print("=== End Coral Bleaching SST Anomaly Check ===")
